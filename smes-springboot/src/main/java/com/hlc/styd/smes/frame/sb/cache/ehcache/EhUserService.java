package com.hlc.styd.smes.frame.sb.cache.ehcache;
...
/**
  ...
  4.在spring boot中通过@EnableCaching注解自动化配置合适的缓存管理器。
        ...
        可以直接在resources下配置ehcache.xml，spring boot会自己侦测到。但是为了后期的演示，我们将其放到config文件夹中，并且改名为ehcache-config.xml,然后配置指定这个xml    spring.cache.ehchache.config=classpath:cofnig/ehcache-config.xml

 常见错误：

        CacheExpressionRootObject
            不能写成@Cacheable(key="findAll") 而应该写成@Cacheable(key="'findAll'")


 */
@CacheConfig(cacheNames="ehusercahe")
@Service("ehuserService")
public class EhUserService implements IUserService<Db_User> {
    ...
    @CachePut(cacheNames = "ehusercahe",key = "#p0.id")
    @Override
    ...
    @CacheEvict(cacheNames = "ehusercahe")
    @Override
